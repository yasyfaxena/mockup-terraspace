...
function extractSqlState(err) {
  ...
  const errorLike = /** @type {{ meta?: { code?: unknown }, message?: unknown }} */ (err);
  if (typeof errorLike.meta?.code === "string") return errorLike.meta.code;
  if (typeof errorLike.message === "string") {
    const match = errorLike.message.match(/code:\s*"([0-9A-Z]{5})"/);
...
/**
 * @param {unknown} err
 * @returns {import("./app-error.js").AppError | null}
 */
...
/**
 * Raw PostgreSQL codes for constraints Prisma has no dedicated mapping for
 * — e.g. the hand-written `payments_one_paid_per_booking` partial unique
 * index (error-handling.md §8).
 * @param {unknown} err
 * @returns {import("./app-error.js").AppError | null}
 */
function mapRawSqlState(err) {
...
export function mapPrismaError(err) {
  const known = mapKnownRequestError(err);
  ...
  return mapRawSqlState(err); // null → unrecognized, InternalError in the handler
}
