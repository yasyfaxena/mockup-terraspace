...
| Doc | Change |
|---|---|
| [`erd-spec.md`](./erd-spec.md) | `admin_settings.currency` and `bookings.currency` default `'IDR'`; `locations.timezone` added |
| [`features/README.md`](./features/README.md) | Currency section — IDR, exponent `0`, the ×100 trap |
| [`features/locations.md`](./features/locations.md) | `timezone` in responses and the create/update field table |
| [`features/settings.md`](./features/settings.md) | `currency: "IDR"`, `currencyExponent: 0` |
| All feature docs | Money examples restated in rupiah — `Rp 50.000`/hour, `Rp 166.500` total |
...
- [ ] `admin_settings` seeded with `currency = IDR`, `tax_percent = 11.00`
...
- [ ] IDR `100000.00` → `100000`; USD `166.50` → `16650`. Unknown currency **throws**
...
| Guest feature ambiguity stalls P5 | P5 | Decision #0.2 before P5 starts; the ERD schema is ready either way |
