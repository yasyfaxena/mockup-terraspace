...
## Phase 8 — Hardening & cutover 🟡 partially done — scope deliberately narrowed

**Before building anything, this phase's own written plan turned out to assume an architecture V2 doesn't have.** "Remove `VITE_API_MODE`" and "delete the `legacy` branch in `lib/api-client.ts`" describe a dual-mode FE that shares one codebase with V1 behind a feature flag — that was superseded by decision #1 (`FE/` is a wholly separate app) before Phase 0 even started. `lib/api-client.ts` has never had a `legacy` branch or a `VITE_API_MODE` to remove; there was nothing to do here. "Delete `src/backend/api/*`" is real, but it's the root V1 app's source (`src/backend/`, `src/frontend/` — a different, still-`git`-tracked tree, not anything under `FE/`), and deleting a live app's source is exactly the kind of repo-wide, hard-to-reverse action this project's own operating rules say to confirm before doing — asked, and **the answer was not yet**. It's carried forward as the one exit criterion this pass leaves unchecked on purpose, not one that was overlooked.

The rest of the phase — real infrastructure that genuinely didn't exist — was also more than one pass could responsibly finish blind, so scope was explicitly agreed with the user up front rather than silently narrowed or silently attempted in full: ship the EN/ID error catalog, error boundaries, and an accessibility pass now; stand up MSW and Playwright as real, working foundations with a couple of real tests each; leave a full REG-FE catalog, a real-BE contract-test suite, and CI wiring (Lighthouse included) as explicit, named follow-up rather than claim them done.

**What shipped:**
- **EN/ID error catalog** — `shared/i18n.tsx` gained `errorMessages: Record<Locale, Record<ErrorCode, string>>`, all 24 codes in `shared/error-codes.ts` translated into both languages, plus `getErrorMessage(code, locale, fallback)`. `lib/query-client.ts`'s global handler now calls it (hardcoded to `"en"` — `I18nProvider` hardcodes the same locale today, and this call sits in the query cache's own `onError`, outside the React tree, so it can't reach `useI18n()` regardless). `tests/unit/shared/error-codes.test.ts` iterates `Object.values(ERROR_CODE)` against both languages — exactly error-handling.md §10's specified test — plus a second test catching a stray catalog key that doesn't correspond to a real code.
- **Error boundaries per route group** — `src/lib/error-capture.ts` (new; a console-based chokepoint, not the Sentry SDK — there is no error-tracking service wired into this app, matching how `libraries.md` describes the V1 utility of the same name) is the one place every boundary reports to. `__root.tsx` gets a site-wide `errorComponent`; `admin.tsx` gets its own, rendered inside `AdminShell` so the sidebar survives a child route's error instead of the whole admin panel going blank.
- **Accessibility pass, scoped to the booking flow** (sign-in/sign-up → workspace detail's slot picker → review → checkout → confirmation → dashboard's cancel dialog, matching the build item's own wording, not swept across the admin catalog forms): every dynamic error/status message that wasn't already inside a toast now carries `role="alert"` or `role="status"`/`aria-live="polite"` (`booking-slot-picker.tsx`'s min-duration message, `booking.review.tsx`'s conflict banner, `checkout-redirect.tsx`'s error and loading states, `payment-status-poller.tsx`'s paid/pending/failed states — this last one matters most, since it's a poller updating with no user action at all), and `sign-in-form.tsx`/`sign-up-form.tsx`'s field errors gained `aria-invalid`/`aria-describedby` wiring to their message. The global toast path (the majority of errors) already had this for free — `sonner` renders its own accessible live region, nothing to add there.
- **MSW foundation** — `tests/mocks/{server.ts,handlers/settings.handlers.ts}`, a new `tests/setup/integration-setup.ts`, and a `vitest.config.ts` restructured into `unit`/`integration` projects (mirroring BE's own `vitest.config.js` shape) so integration specs run under `jsdom` while unit specs stay `node`. Two real integration specs prove the foundation actually works, not just compiles: `tests/integration/error-handling/global-error-handler.test.tsx` (a real fetch through `lib/api-client.ts`, a real `ApiError`, the real query-cache `onError`, the real `errorMessages` catalog — MSW is the only fake part, and a second case proves the unknown-code fallback to `error.message`) and `tests/integration/features/public-settings.test.tsx` (loading → success → a MSW-overridden response → a real 500 → error state, on the real `usePublicSettings()` hook).
- **Playwright, installed and actually run, CI wiring left for later** (per direct instruction mid-phase) — `@playwright/test` + chromium, `playwright.config.ts` (no `webServer` entry — it assumes `BE/`/`FE/` are already running locally, same as every manual verification pass this whole project has used since Phase 5; wiring a CI-managed `webServer` is the named follow-up), and `tests/e2e/regression/REG-FE-001-ssr-query-client-cache-leak.spec.ts` — a real regression guard for Phase 5's most severe defect (the module-level `QueryClient` singleton), reproduced against `/admin/locations` instead of the calendar it was originally found on since it needs no booking/pricing fixtures and exercises the exact same `createQueryClient()`-per-request contract the fix lives in. `tests/e2e/helpers/{auth.ts,db.ts}` sign up + directly verify/promote a real test user (the same bootstrap problem `BE/tests/helpers/auth.js` solves the same way, for the same reason: Better Auth requires a verified email before sign-in, and there's no inbox to click a real link) — genuinely run against the live dev servers, not just written: failed twice first (once on an unrelated transient `ECONNRESET`, once because the bare `request` fixture doesn't share cookies with `page` — only `page.request` does — both real findings from actually running it), then passed, then re-run clean after fixing a `noPropertyAccessFromIndexSignature` TS error the first green run hadn't hit yet.

**Deliberately not attempted this pass, named rather than silently skipped:**
- Deleting `src/backend/`/`src/frontend/` (the root V1 app) — asked, user said not yet.
- A full `REG-FE-*` catalog — only REG-FE-001 exists. The other real candidates from this project (the Phase 5 settings-prefetch gap, the Phase 6 payment-status-prefetch gap, the Phase 7 payments-report-tab prefetch gap) are all genuine, all fixed, and all still without a spec.
- A router+session MSW integration harness (a `renderRoute()` test helper that also mocks Better Auth's `get-session` endpoint) — the two integration specs that exist deliberately avoid needing one; testing.md's own `booking-review.test.tsx` example needs it and doesn't exist yet.
- A real-`BE/` contract-test suite (testing.md §7) and a `test:e2e` CI job (including Lighthouse) — infrastructure/CI decisions, not just code, left for a pass where that's the explicit focus.

### Exit criteria

- [ ] `src/backend/`, `src/frontend/` no longer exist in the tree — **deliberately left unchecked**: a repo-wide, hard-to-reverse deletion of the live V1 app, asked about explicitly, deferred by the user's own answer rather than decided unilaterally
- [x] `npm run check` clean: lint, types, format — verified after every change in this phase, not just at the end
- [ ] Lighthouse accessibility ≥ 90 on the booking review screen — not attempted; no Lighthouse CI wiring exists yet. The accessibility *pass* itself (labels, `aria-live`, keyboard nav via already-accessible Radix primitives) was done and is a real improvement, just not scored
- [x] Every toast-producing `ERROR_CODE` has both an EN and ID string — `errorMessages` covers all 24 codes in both languages, wired into the actual global handler, proven by a coverage test and by a real integration test asserting the translated (not raw) string reaches `toast.error`
- [ ] E2E suite green against a real (non-mocked) `BE/` instance in CI — the suite exists and is green against a real `BE/` (confirmed by actually running it, twice catching real bugs in the test itself along the way), but there is no CI job running it yet — that wiring was explicitly deferred, not the running-it-for-real part

Verified against real running `BE/`/`FE/` dev servers throughout: `npm run check`, `npm run test` (19 tests: 14 unit + 5 new integration, all real assertions against MSW-faked responses), `npm run build`, and `npx playwright test` (1 passed, against the real live servers, with its own test user/location created and cleaned up via the same sign-up → verify-via-DB → sign-in pattern this whole project has used since Phase 5).
