...
export function AdminSettingsForm() {
  ...
  return (
    <form onSubmit={(e) => void handleSubmit(onSubmit)(e)} className="max-w-lg space-y-4">
      ...
      <div className="grid gap-1.5">
        <Label htmlFor="settings-hero-headline" className={labelClass}>
          Hero headline
        </Label>
        <Input
        ...
        />
        {errors.heroHeadline && (
          <p className="text-xs text-destructive">{errors.heroHeadline.message}</p>
        )}
        <p className="text-[11px] text-white/35">
          Shown as the eyebrow text above the headline on the homepage hero, e.g. “TerraSpace ·
          Johor Bahru”.
        </p>
      </div>
      ...
      <div className="grid gap-1.5">
        ...
        {errors.heroBackgroundUrl && (
          <p className="text-xs text-destructive">{errors.heroBackgroundUrl.message}</p>
        )}
        <p className="text-[11px] text-white/35">
          Used as the homepage hero background — paste a URL or upload a file. Leave empty to
          fall back to the plain hero background.
        </p>
      </div>
