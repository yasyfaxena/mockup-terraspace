...
export const updateSettingsSchema = z.object({
  ...
  // No `.max()` — this can be a pasted URL or a `data:` URI from an
  // uploaded file (mirrors workspaces/locations `imageUrl`, which are
  // unbounded for the same reason; BE's column matches).
  heroBackgroundUrl: z.string().trim().url().nullable(),
  currency: z
    .string()
    .length(3)
    .toUpperCase()
