...
function AdminBookingsPage() {
  ...
  async function handleDelete(booking: AdminBookingListItemDto) {
    ...
    try {
      ...
      // A settled payment is the one case the API refuses outright. Offer
      // the override here instead of leaving the admin stuck on a 409.
      if (!(err instanceof ApiError) || err.code !== ERROR_CODE.BOOKING_HAS_SETTLED_PAYMENT) {
        return; // query-client's global handler already toasted the real message
      }
    }

    if (
      !confirm(
        `${booking.reference} has a paid or refunded payment. Deleting it also destroys that payment record, and it will disappear from revenue reports. Continue?`,
      )
    ) {
      return;
    }
    ...
    try {
      await deleteBooking.mutateAsync({ id: booking.id, force: true });
      toast.success(`Booking and payment records deleted — ${booking.reference}`);
    } catch {
      // query-client's global handler already toasts the real ApiError message
    }
  }
